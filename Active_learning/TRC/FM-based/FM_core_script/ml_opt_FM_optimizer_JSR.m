...
if strcmp(input.fm_optimizer,'simulated_annealing')
    ...
    SA_name = sprintf('%s_cal_fm_SA_time_%d.mat',filename_header,ttn);
    save(SA_name, 'caltime');

    caltime=[caltime_fm_array;caltime_SA_array];
    SA_name = sprintf('cal_fm_SA_time_%d.mat',ttn);
    save(SA_name, 'caltime');
...
elseif strcmp(input.fm_optimizer,'quantum_annealing')
    ...
    [QUBO_for_qlm,Bias_for_qlm]=formulate_QUBO_FM_JSR(inputFM);
    QA_time= tic;
    [os,oi]=system(sprintf('C:\\Users\\user\\miniconda3\\envs\\JSR_jupyter_desktop\\python -c "from qubo_dimod import qubo_run; qubo_run(%s)"',[char(39) fntd char(39)]));        %% Predict Global minimum 

    caltime_QA=toc(QA_time);
    caltime_QA_array(in-24)=caltime_QA;
    caltime=[caltime_fm_array;caltime_QA_array];

    QA_name = sprintf('%s_cal_fm_QA_time_%d.mat',filename_header,ttn);
    save(QA_name, 'caltime');
...
elseif strcmp(input.fm_optimizer,'tensor_brute_force')
    ...
    for jn=1:junk_number
        ...
        inputGFM.w2=inputFM.w2;
        inputGFM.w1=inputFM.w1;
        inputGFM.w0=inputFM.w0;
        FOM_predict_dummy=FM_main_JSR(inputGFM,2);
    ...
    fm_time_name = sprintf('cal_fm_brute_time_%d.mat',ttn);
    save(fm_time_name,"caltime");
    
elseif strcmp(input.fm_optimizer,'hybrid_quantum_annealing')
        %% Predict Global minimum with Quantum Annealing using d:wave cloud service
    % Formulate QUBO matrix for myQlm
    [QUBO_for_qlm,Bias_for_qlm]=formulate_QUBO_FM_JSR(inputFM);
    QA_time= tic;
    [os,oi]=system(sprintf('C:\\Users\\user\\miniconda3\\envs\\JSR_jupyter_desktop\\python -c "from QUBO_HBA import d_Hsolver; d_Hsolver(%s)"',[char(39) fntd char(39)]));        %% Predict Global minimum 

    caltime_QA=toc(QA_time);
    caltime_QA_array(in-24)=caltime_QA;
    caltime=[caltime_fm_array;caltime_QA_array];
    HQA_name = sprintf('%s_cal_fm_HQA_time_%d.mat',filename_header,ttn);
    save(HQA_name, 'caltime');
